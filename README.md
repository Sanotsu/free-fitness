...
此 App 适合拥有运动健身、减肥增肌、饮食记录、随手记等需求的辅助使用。所有数据全部在本地。
...
- 2025-08-01: 为方便直接使用，`0.2.2-beta.1`及其之后可能存在的版本，会内置部分数据：
  ...
  - **注意**：仅全新初次使用才会初始化内置数据，App 未卸载的直接升级或覆盖恢复，不会初始化内置数据
    ...
    - 2026-08-31: **更新内置食物成分数据为最新版本**

## 更新

- 2026-08-31 `0.2.3-beta.1`
  - 全新"智能助手"AI 模块：自配大模型 API、内置 5 个专属角色+自定义角色管理、饮食/餐照/训练/计划业务分析收口
  - 备份恢复支持按模块(用户/饮食/手记/训练/AI)选块合并恢复，带进度展示
  - 更新内置食物成分数据为最新版本
- 2025-08-01 `0.2.2-beta.1`
  - 内置部分“基础动作”和“食物成分”数据，方便直接使用
- 2024-12-03 `0.2.1-beta.1`
  - 主要接入零一万物 AI 大模型 API，对饮食记录和餐食照片进行 AI 分析和问答式提供建议。

更多改动参看 [CHANGELOG](CHANGELOG.md)

**注意，如果要卸载或者升级版本，建议先进行一次全量备份，再次安装时首先进行合并恢复。**
...
- 注意，需要手机有安装 TTS 引擎才有语音提示；无 TTS 引擎的设备也可以正常跟练，只是没有语音(进入跟练页时会提示一次)
- 训练组列表、计划列表及计划内每日训练会显示"预估耗时"，按动作配置与间隔休息设置实时估算，口径与跟练执行一致
...
- 会将当日摄入的食物以及营养素传入，由"智能助手"以营养师角色进行分析，支持简单多轮对话
- 同一天的摄入数据未变更时，再次进入仅查看之前的分析，不会重复调用大模型
...
在饮食记录的照片页面或在餐食相册页面，也可以进行 AI 图像分析

- 由"智能助手"以营养师角色分析，支持一次最多 4 张图片
- 需要"大模型配置"中启用的模型支持视觉能力(配置页有"测试视觉"可验证)，未变更的同一餐次图片不会重复调用
...
`0.2.3-beta.1` 新增的通用 AI 助手，主页悬浮按钮即可进入，各业务入口(饮食日记/餐食照片/训练组/周期计划)也会带上下文跳转进来。

- **大模型 API 自行配置**：在`"用户与设置"-"更多设置"-"大模型配置"`中添加任一 OpenAI 兼容平台的接口地址与密钥，支持保存多套配置随时切换；每个配置独立选择模型、是否支持视觉、高级参数，并提供"测试连接"/"测试视觉"
- **角色系统**：内置通用助手、健康助手、营养师、健身教练、体重管理顾问 5 个角色(各自有独立设定与建议问题)；内置角色可查看不可修改，另可新增自定义角色
- **通用对话**：流式响应、多图上传(最多4张)、会话历史管理(切换/重命名/清空/删除)、token 用量展示
- **业务会话复用**：同一业务对象(同一天饮食/同一餐次照片/同一训练组/同一计划)复用同一会话，数据未变更时仅查看之前的分析，变更后自动追加一次新分析
- **备份恢复**：AI 配置与会话图片包含在模块化备份恢复中

## 用户与设置

主要就是一些用户信息管理，体重记录管理，每日摄入目标，备份恢复，切换语言和主题之类的。

其中备份是把内置数据库的数据全部导出为 json 文件后构建一个 zip 压缩包，后续其实不一定是真的 app 备份的 zip 文件，只要格式一致，自行压缩的 zip 文件也可以恢复。

`0.2.3-beta.1`起恢复时可以**按模块(用户/饮食/手记/训练/AI)勾选合并恢复**，只恢复需要的部分；AI 的大模型配置与会话图片也随备份恢复。
...
- 2026-08-31: `0.2.3-beta.1` 起 AI 平台由用户自行配置(任一 OpenAI 兼容平台)，未配置 AI 时各功能正常使用，只是 AI 分析入口会引导前往配置。
